...
class OptimizationProblem:
    """
    Optimization problem for splitting energy of a single input between multiple targets.
    """
    ...
    def setup_objective(self) -> None:
        ...
        target_blocks_list = [
            ...
            for target_size, target_shift in zip(target_sizes, target_shifts)
        ]

        # Array of kinetic energies for each target
        def target_kinetic_energies(horizontal_vertical_shifts: Tuple[jnp.ndarray, jnp.ndarray]):
            ...
            return jnp.array([
                kinetic_energy(
                    block_velocity=solution_data.fields[:,
                                                        1, target_blocks, :],
