...
from difflexmm.geometry import KagomeGeometry, compute_inertia, compute_edge_angles, compute_edge_lengths
...
class ForwardProblem:
    """
    Forward problem for the kagome focusing problem.
    BCs:
        - Clamped corners.
    ...
    def setup(self, excited_blocks_fn=None) -> None:
        ...
        # Geometry
        geometry = KagomeGeometry(
            n1_cells=self.n1_cells,
            n2_cells=self.n2_cells,
            direct_basis=self.cell_size *
            jnp.array(
                [[1., 0.], [jnp.cos(self.cell_angle), jnp.sin(self.cell_angle)]]),
        ...
        if self.loaded_side == "left":
            driven_block_DOF_pairs = jnp.array([
                jnp.tile(
                    jnp.arange(2*geometry.n1_cells*((geometry.n2_cells - n_excited_blocks)//2), 2 *
                               geometry.n1_cells*((geometry.n2_cells + n_excited_blocks)//2), 2*geometry.n1_cells), 3),
                jnp.array([0]*n_excited_blocks + [1] *
                          n_excited_blocks + [2]*n_excited_blocks)
            ]).T
        else:
            raise ValueError(
                f"Unknown loaded_side: {self.loaded_side}. Only 'left' is implemented.")
        # Clamped corners
        n_blocks_clamped_corner = self.n_blocks_clamped_corner
        clamped_block_DOF_pairs_bl = jnp.array([
            jnp.tile(
                jnp.concatenate([
                    jnp.arange(0, n_blocks_clamped_corner),
                    jnp.array(
                        [0+i*geometry.n1_cells for i in range(1, n_blocks_clamped_corner)])
                ]), 3) * 2,
        ...
        clamped_block_DOF_pairs_br = jnp.array([
            jnp.tile(
                jnp.concatenate([
                    jnp.arange(geometry.n1_cells -
                               n_blocks_clamped_corner, geometry.n1_cells)*2,
                    jnp.array([(i+1)*2*geometry.n1_cells -
                              1 for i in range(0, n_blocks_clamped_corner)])
                ]), 3),
            jnp.array([0]*(2*n_blocks_clamped_corner) + [1] *
                      (2*n_blocks_clamped_corner) + [2]*(2*n_blocks_clamped_corner))
        ]).T
        clamped_block_DOF_pairs_tr = jnp.array([
            jnp.tile(
                jnp.concatenate([
                    jnp.arange(geometry.n_cells-n_blocks_clamped_corner, geometry.n_cells), jnp.array(
                        [geometry.n_cells-i*geometry.n1_cells-1 for i in range(1, n_blocks_clamped_corner)])
                ]) * 2 + 1, 3),
            jnp.array([0]*(2*n_blocks_clamped_corner-1) + [1] *
                      (2*n_blocks_clamped_corner-1) + [2]*(2*n_blocks_clamped_corner-1))
        ]).T
        clamped_block_DOF_pairs_tl = jnp.array([
            jnp.tile(
                jnp.concatenate([
                    jnp.arange(geometry.n_cells-geometry.n1_cells, geometry.n_cells -
                               geometry.n1_cells+n_blocks_clamped_corner)*2+1,
                    jnp.array([geometry.n_cells-geometry.n1_cells-i *
                               geometry.n1_cells for i in range(0, n_blocks_clamped_corner)]) * 2
                ]), 3),
            jnp.array([0]*(2*n_blocks_clamped_corner) + [1] *
                      (2*n_blocks_clamped_corner) + [2]*(2*n_blocks_clamped_corner))
        ]).T
        ...
        # Setup forward
        def forward(shifts: Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]):
...
class OptimizationProblem:
    ...
    def setup_objective(self) -> None:
        ...
        target_blocks = jnp.array([
            (2*(j * geometry.n1_cells + i), 2*(j * geometry.n1_cells + i)+1)
            for i in range((geometry.n1_cells-target_size[0])//2 + target_shift[0], (geometry.n1_cells+target_size[0])//2 + target_shift[0])
            for j in range((geometry.n2_cells-target_size[1])//2 + target_shift[1], (geometry.n2_cells+target_size[1])//2 + target_shift[1])
        ]).flatten()

        def target_kinetic_energy(shifts: Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]):

            # Solve forward
            solution_data = self.forward_problem.solve(shifts)
